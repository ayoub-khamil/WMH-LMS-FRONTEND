WatermelonHub LMS — BPO Training Platform (wmh-lms-frontend)
1. Overview & Tech Stack
Overview: Frontend demo / prototype for WatermelonHub LMS, a role-based BPO training platform. Two experiences in one SPA:
...
Seed demo users (src/services/mockData.js:1):
...
Routing Logic (appRoutes.js + App.jsx:247):
...
*	redirect home	—
isAllowedPath() prevents agents from hitting /manager/*. paths.*() helpers centralize URL construction. HashRouter chosen for static-file demo deployability (no server rewrites).
...
No env vars required. Reset demo data by clearing localStorage keys wmh_lms_* and reloading (reseeds from mockData.js).
Login with seed users above. Publish course 103 (draft) to make it visible to agents; assign it under Manager → Assignments.
